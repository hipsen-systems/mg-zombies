...
- All three workflows that need the engine get it from the `.github/actions/setup-godot` composite action, which is also **the one place the CI Godot version is pinned**. Bump it there; it used to be an `env` var repeated in each workflow, so a bump had three places to miss (issue #10).
